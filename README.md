...
- **双存储模式**：元数据存储支持 **H2 嵌入式库**（单机零依赖，默认）与 **MySQL**（集群多副本），两种模式平滑切换。
...
| 模式 | 适用场景 | 说明 |
| :--- | :--- | :--- |
| `h2` (默认) | 单机 / 开发测试 | 嵌入式 H2 文件库，零外部依赖，开箱即用；无冗余 API，单副本写入 |
...
4. **一致性约束**：
   ...
   - 集群部署时**不应使用** `h2` 模式（H2 为单机库，多 NameNode 实例会导致 Brain Split）。
...
- **Dashboard 显示"冗余 API 未启用"**：`storage.mode` 为 `h2`；切换 `mysql` 后启用。
